...
An Elegant wrapper around Symfony's Process component with full test coverage.
...
To execute a command, you may use the `execute` method. First, let's examine how to execute a basic shell command.
...
The `execute` method returns an instance of `TitasGailius\Terminal\Response`, which provides a variety of methods that may be used to inspect the response:
```php
$response->code() : int;
$response->ok() : bool;
...
Terminal::inBackground()->execute('rm -rf node_modules');
...
```php
Terminal::in(storage_path('framework'))->execute('rm -rf views');
```
...
```php
Terminal::timeout(25)->execute('rm -rf vendor');
```
...
Terminal::timeout($duration)->execute('rm -rf vendor');
```
...
Terminal::timeout($date)->execute('rm -rf vendor');
```
...
Terminal::extend('removeVendors', function ($terminal) {
  return $terminal->execute('rm -rf vendors');
});
...
Terminal::execute('php artisan migrate');
...
$this->assertEquals(123, Terminal::execute('factor 12')->getPid());
$this->assertEquals(321, Terminal::execute('php artisan migrate')->getPid());
```
